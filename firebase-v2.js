(function(root){
  ...
  async function ensurePlayerAccount({user,displayName="",clubId="",joinCode=""}={}){
    ...
    const profile=root.LaTeamClubV2.playerProfile({playerId:existingPlayerId||undefined,ownerUid:user.uid,displayName:name,now}),batch=db.batch();batch.set(db.collection("players").doc(profile.playerId),profile);
    const mutableUser={schemaVersion:2,displayName:profile.displayName,defaultClubId,playerId:profile.playerId,updatedAt:now};
    if(existingDoc.exists)batch.update(userRef,mutableUser);else batch.set(userRef,{...mutableUser,email:root.LaTeamAccounts.normalizeEmail(user.email),createdAt:now});
