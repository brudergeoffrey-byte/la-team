(function(root){
  ...
  async function createPlayerAccount({email,password,displayName,clubId,joinCode}){
    ...
    const profile=root.LaTeamClubV2.playerProfile({ownerUid:user.uid,displayName}),clubPlayer=root.LaTeamClubV2.clubPlayer({player:profile,clubId,joinedWithCode:String(joinCode||"").toUpperCase()}),now=Date.now(),batch=db.batch();
    batch.set(db.collection("players").doc(profile.playerId),profile);batch.set(db.collection("users").doc(user.uid),{schemaVersion:2,email:normalizedEmail,displayName:profile.displayName,defaultClubId:clubId,playerId:profile.playerId,createdAt:now,updatedAt:now});batch.set(db.collection("clubs").doc(clubId).collection("players").doc(profile.playerId),clubPlayer);await batch.commit();return {user,profile,clubId};
  }
  ...
  root.LaTeamV2Cloud={createPlayerProfile,getMyPlayerProfile,getMyMatchHistory,addPlayerToClub,searchClubPlayers,listClubPlayers,createSeason,listSeasons,createEvent,listEvents,ensureClubJoinCode,createPlayerAccount,signIn,signOut,currentContext,listPlayerEvents,subscribeEvents,subscribeRegistrations,getMyRegistration,registerPlayerSpark,cancelRegistrationSpark,addGuestSpark,listRegistrations,createParticipants,linkEventTournament,getMyEventParticipant,registerPlayer,addGuest,cancelRegistration,linkGuestRegistration,submitScoreProposal,listScoreProposals,subscribeScoreProposals,saveOfficialMatch,reviewScoreProposal,recordHistoricalCorrection,saveRoundSummary};
