...
assert.match(html,/NextPadel — environnement TEST · aucune donnée de production/);assert.match(html,/Créer des données de démonstration/);assert.match(html,/function createDemoData\(\)/);
assert.equal(manifest.name,"NextPadel — TEST");assert.equal(rc.projects.preproduction,"la-team-v2-test");assert.ok(firebase.hosting.ignore.includes("functions/**"));assert.ok(firebase.hosting.ignore.includes("tests/**"));
