...
assert.equal(context.LA_TEAM_ENV.name,"preproduction");assert.equal(context.LA_TEAM_ENV.firebaseConfigured,false);assert.equal(context.LA_TEAM_ENV.demoEnabled,true);
assert.equal(context.LA_TEAM_ENV.firebaseConfig.projectId,"la-team-v2-test-unconfigured");
...
assert.equal(manifest.name,"La Team V2 — TEST");assert.equal(rc.projects.preproduction,"la-team-v2-test-unconfigured");assert.ok(firebase.hosting.ignore.includes("functions/**"));assert.ok(firebase.hosting.ignore.includes("tests/**"));
