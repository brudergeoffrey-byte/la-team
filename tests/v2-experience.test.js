...
for(const marker of ["Le padel.","Ensemble.","Rejoindre un tournoi","Créer et gérer","Suivre en direct","TOURNOIS À VENIR","Pourquoi NextPadel ?","Organisation simple","En temps réel","Équitable","Sur tous vos écrans","ESPACE JOUEUR","ESPACE ORGANISATEUR","ACCÈS VIEWER","Accueil","Tournois⌄","Mon club⌄","Profil","Tableau de bord","Joueurs","Matchs & terrains","Paramètres","Créer un événement","Bonjour ${p.displayName}","S’INSCRIRE","✓ INSCRIT","Mes statistiques","PRÉPARER LE TOURNOI","OUVRIR LE MODULE TOURNOI","AUCUN PAIEMENT"]){assert.ok((html+source).includes(marker),`expérience V2 présente : ${marker}`);}
...
for(const marker of ["Bienvenue 👋","SE CONNECTER","CRÉER MON COMPTE","Gérez votre club avec NextPadel","CRÉER MON CLUB","facultatif"]){assert.ok(source.includes(marker),`entrée authentification explicite : ${marker}`);}
