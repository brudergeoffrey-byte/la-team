...
for(const file of ["offline.html","bg.jpg","icons/icon-180.png","icons/icon-192.png","icons/icon-512.png","icons/icon-maskable-512.png"]){
...
(async()=>{
  ...
  const updatedSource=swSource.replace("la-team-shell-v1","la-team-shell-v2");
  vm.runInContext("(function(){"+updatedSource+"\n})()",context);online=true;
  await dispatchLifecycle("install");await dispatchLifecycle("activate");
  assert.deepEqual([...stores.keys()],["la-team-shell-v2"],"ancien cache applicatif nettoyé");
