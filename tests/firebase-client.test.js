...
function harness(url="https://brudergeoffrey-byte.github.io/la-team/",authUid="owner-1"){
  ...
  const makeCollection=path=>({
    doc(id){return makeDoc(`${path}/${id}`,path==="tournaments");},
  ...
  const makeDoc=(path,rootDoc=false)=>{const code=path.split("/")[1];return {_path:path,_code:code,_root:rootDoc,
    async set(value){if(!navigator.onLine)throw new Error("offline");storeAt(this,value);},
  ...
  const db={
    ...
    collection(name){assert.equal(name,"tournaments");return makeCollection(name);},
  ...
  const auth={currentUser:null,async signInAnonymously(){authCalls++;this.currentUser={uid:authUid};return {user:this.currentUser};}};
